...
class model:
    ...
    def batch_loss(self, labels):
        ...
        # Calculating loss
        if "EmbeddingLoss" in self.criterions.keys():
            self.loss_embed, self.loss_embed_proto, self.loss_embed_biasreduc = self.criterions["EmbeddingLoss"](self.embedding, labels)
            self.loss_embed  *= self.criterion_weights["EmbeddingLoss"]
            self.loss += self.loss_embed 
        ...
        if "ClassifierLoss" in self.criterions.keys():
            ...
            self.loss += self.loss_classifier        #------Note here that it is not +=. GPU memory saving measure!!!!
    ...
    def train(self, retrain=False):
        ...
        for epoch in range(self.start_epoch, self.end_epoch + 1):
            ...
            g.epoch_global = epoch  #---global config
            ...
            for inputs, labels, indexes in self.data["train"]:               
                ...
                time1_1 = time.time()
                step_time =  time1_1 - time1
                time1 = time1_1

                wandb_log({"Loader": et_dataloader,
                        "ForBack": et_forback,
                        "Total": (et_dataloader+et_forback),
                        "Step_time": step_time,
                        "Epoch Elap": (time1 - epoch_start_time)/60.0,})
            ...
            if "test" in list(self.data.keys())[1:]:
                rsls_test, _ , _ , _ = self.eval(phase='test')
                rsls.update(rsls_test)
                del rsls_test
            ...
            wandb_log({"Eval Elap": (eval_time2-eval_time1)/60.0, 
                    "Epoch Elap": (time.time() - epoch_start_time)/60.0})
    ...
    def save_latest(self, epoch):
        """Saves latest weights of the model
