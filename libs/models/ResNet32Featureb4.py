...
class BBN_ResNet_Cifar(nn.Module):
    ...
    def forward(self, x, retrain,**kwargs):
        
        if retrain:
            outb4 = x.reshape(x.shape[0], 64, 8, 8)
        else:
            out = F.relu(self.bn1(self.conv1(x)))
            out = self.layer1(out)
            out = self.layer2(out)
            outb4 = self.layer3(out)
        if "feature_cb" in kwargs:
            outb4 = self.cb_block(outb4)
            return out
        elif "feature_rb" in kwargs:
            outb4 = self.rb_block(outb4)
            return out

        out1 = self.cb_block(outb4)
        out2 = self.rb_block(outb4)
        ...
        return outb4.flatten(start_dim=1), out
    
    # def forward(self, x, **kwargs):
    #     outb1 = F.relu(self.bn1(self.conv1(x)))
    #     outb2 = self.layer1(outb1)
    #     outb3 = self.layer2(outb2)
    #     outb4 = self.layer3(outb3)
    #     if "feature_cb" in kwargs:
    #         out = self.cb_block(outb4)
    #         return out
    #     elif "feature_rb" in kwargs:
    #         out = self.rb_block(outb4)
    #         return out
    ...
    #     outb5 = torch.cat((out1, out2), dim=1)

    #     outb5avg = self.avgpool(outb5)
    #     out = outb5avg.view(outb5avg.shape[0], -1)

    #     return outb1,outb2,outb3,outb4,outb5,outb5avg, out
...
# a = create_model(
#     pretrain=True, pretrain_dir="libs/models/sample_final_model_checkpoint.pth"
# )
# print(a)
